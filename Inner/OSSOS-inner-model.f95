...
!
! P(a) follows CFEPS inner belt model with P(a) \propto a^{-2.5}, between
! 37.7 au (modified from CFEPS) and 39 au.
!
! P(q) follows CFEPS inner belt model, with
! P(q) \propto 1/([1+exp((35-q)/1)][1+exp((q-40)/1)]), limited to range [33; 40]
! 
...
! Calibrated number of objects
    data &
         n_calib /1550/,          &! Number of object at H_calib
         h_calib /8.66d0/
...
!       read (lun_m, *) (param(comp*10+i),i=1,4)
       param(comp*10+1:comp*10+4) = [40.0d0, 1.0d0, 35.0d0, 1.0d0]
...
! H-mag distribution: Exponential cutoff and broken exponential law
    h = H_draw_hot_5(seed, n_h, h_params(0*n_h+1))
!
